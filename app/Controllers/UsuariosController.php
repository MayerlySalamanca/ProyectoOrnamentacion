...
class UsuariosController
{
    ...
    public function __construct(array $_FORM)
    {
        ...
        $this->dataUsuario['roll'] = $_FORM['roll'] ?? "administrador";
    ...
    static public function selectUsuario(array $params = []) {
        ...
        $arrUsuarios = array();
        if ($params['where'] != "") {
            $base = "SELECT * FROM ornamentacion.usuario WHERE ";
        ...
        if (is_array($arrUsuarios) && count($arrUsuarios) > 0) {
            ...
            foreach ($arrUsuarios as $usuario)
                if (!UsuariosController::usuarioIsInArray($usuario->getIdUsuario(), $params['arrExcluir']))
                    $htmlSelect .= "<option " . (($usuario != "") ? (($params['defaultValue'] == $usuario->getIdUsuario()) ? "selected" : "") : "") . " value='" . $usuario->getIdUsuario() . "'>" . $usuario->getDocumento() . " - " . $usuario->getNombres() . "</option>";
        }
        $htmlSelect .= "</select>";
        return $htmlSelect;
    }
    ...
    public static function login (){
        try {
            if(!empty($_POST['usuario']) && !empty($_POST['contrasena'])){
                $tmpUser = new Usuario();
                $respuesta = $tmpUser->login($_POST['usuario'], $_POST['contrasena']);
