...
CREATE TABLE IF NOT EXISTS `ornamentacion`.`Usuario` (
  ...
  `roll` ENUM('empleado', 'cliente') NOT NULL,
...
CREATE TABLE IF NOT EXISTS `ornamentacion`.`Factura` (
  ...
  `usuariosComprador` BIGINT(10) NOT NULL,
  `usuarioVendedor` BIGINT(10) NOT NULL,
  PRIMARY KEY (`IdFactura`),
  INDEX `fk_FacturaVentas_Usuarios1_idx` (`usuariosComprador` ASC),
  INDEX `fk_Factura_Usuario1_idx` (`usuarioVendedor` ASC),
  CONSTRAINT `fk_FacturaVentas_Usuarios1`
    FOREIGN KEY (`usuariosComprador`)
...
CREATE TABLE IF NOT EXISTS `ornamentacion`.`Producto` (
  ...
  `tipoServicio` ENUM("Fabricación de puertas en calibre 18", "Instalación de productos de tipo ornamentación", "Fabricación de ventanas en calibre 18", "Fabricación de portones  en calibre 18", "Instalación de estructuras tipo industrial", "Reparación de productos  de tipo ornamentación") NOT NULL,
...
CREATE TABLE IF NOT EXISTS `ornamentacion`.`MateriaPrima` (
  ...
  `nombre` VARCHAR(45) NOT NULL,
  `tipo` ENUM("Varilla tipo cuadrada y redonda", "Perfiles cuadrados en pulgada y 3/4 calibre 18", "Marco puerta en calibre 18", "División ventana en calibre 18", "Ángulo de pulgada ", "Varilla entorchada", "Pintura en aceite", "Anticorrosivo", "Lija") NOT NULL,
  `stock` INT NOT NULL,
...
CREATE TABLE IF NOT EXISTS `ornamentacion`.`Fabricacion` (
  `idFabricacion` TINYINT(4) NOT NULL AUTO_INCREMENT,
  `cantidad` INT NOT NULL,
  `MateriaPrimaId` INT NOT NULL,
  `Usuario_IdUsuario` BIGINT(10) NOT NULL,
  PRIMARY KEY (`idFabricacion`),
  INDEX `fk_Fabricacion_MateriaPrima1_idx` (`MateriaPrimaId` ASC),
...
CREATE TABLE IF NOT EXISTS `ornamentacion`.`OrdenCompra` (
  ...
  `fabricacionId` TINYINT(4) NOT NULL,
  `Factura_IdFactura` INT NOT NULL,
...
CREATE TABLE IF NOT EXISTS `ornamentacion`.`Pedidos` (
  `idPedidos` TINYINT(3) ZEROFILL NOT NULL AUTO_INCREMENT,
  `nombre` VARCHAR(45) NOT NULL,
  `fechaPedido` DATE NOT NULL,
  `fechaEntrega` DATE NOT NULL,
  `Proveedor_IdProveedor` BIGINT(10) NOT NULL,
...
CREATE TABLE IF NOT EXISTS `ornamentacion`.`DetallePedido` (
  `idDetallePedido` BIGINT(10) NOT NULL AUTO_INCREMENT,
  `valor` INT NOT NULL,
  `cantidad` INT NOT NULL,
  `pedidosId` TINYINT(3) ZEROFILL NOT NULL,
  `materiaPrimaId` INT NOT NULL,
